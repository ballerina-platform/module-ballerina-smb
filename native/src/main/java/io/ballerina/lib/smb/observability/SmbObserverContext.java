...
/**
 * Extension of ObserverContext for the SMB connector.
 * Automatically attaches {@code type}, {@code remote.url}, {@code protocol}, and {@code module} tags.
 */
public class SmbObserverContext extends ObserverContext {
    ...
    SmbObserverContext(String context) {
        addTag(TAG_CONTEXT, context);
        addTag(TAG_MODULE, SmbMetricsUtil.MODULE_SMB);
    }

    public SmbObserverContext(String context, String remoteUrl, String protocol) {
        this(context);
        addTag(TAG_REMOTE_URL, remoteUrl != null ? remoteUrl : SmbMetricsUtil.UNKNOWN);
        addTag(TAG_PROTOCOL, protocol != null ? protocol : SmbMetricsUtil.UNKNOWN);
    }
}
